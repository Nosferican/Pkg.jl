        ...
        mktempdir() do depot_dir
            ...
            try
                ...
                withenv("JULIA_PKG_DEVDIR" => tmp) do
                    ...
                    @test Pkg.dependencies()[Pkg.project().dependencies["UnregisteredWithoutProject"]].version == v"0.0.0"
                    Pkg.test("UnregisteredWithoutProject")
                    Pkg.test("UnregisteredWithProject")
                end
            finally
    ...
    mktempdir() do tmp
        ...
        with_current_env() do
            ...
            Pkg.REPLMode.pkgstr("""
                ...
                test
                """)
            current_json = Pkg.dependencies()[Pkg.project().dependencies["JSON"]].version
            ...
            pkg"up"
            @test Pkg.dependencies()[Pkg.project().dependencies["JSON"]].version.minor == 18
            write("Project.toml", old_project)
            pkg"up"
            @test Pkg.dependencies()[Pkg.project().dependencies["JSON"]].version == current_json
        end
    end
end; end
